...
class VectorStore:
    ...
    def add_document(
        self, doc_id: str, embedding: List[float], document: str, metadata: Optional[Dict[str, Any]] = None
    ):
        """Add a single document and its embedding to the store."""
        self.collection.add(ids=[doc_id], embeddings=[embedding], metadatas=[metadata or {}], documents=[document])
