...
async def checkApiKey(incoming_key: str = Security(api_key_header)):
    ...
    valid_keys = {"server_a_key": "Marketing-Server", "server_b_key": "Analytics-Engine"}
    ...
    if not incoming_key or not server_name:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Unauthorized: Invalid or missing Server API Key"
        )
